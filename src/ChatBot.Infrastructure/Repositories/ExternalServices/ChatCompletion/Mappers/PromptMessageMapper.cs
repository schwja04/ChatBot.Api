...
internal class PromptMessageMapper(IPromptRepository promptRepository) : IPromptMessageMapper
{
    ...
    public async Task<string> BuildMessageContentAsync(ChatMessage message, Guid userId, CancellationToken cancellationToken)
    {
        ...
        Prompt? prompt = await _readPromptRepository.GetAsync(userId, message.PromptKey, cancellationToken);
